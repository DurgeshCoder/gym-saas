...
import { Plus, Utensils, UserPlus } from "lucide-react";
...
interface DietPlan {
    ...
    createdAt: string;
    creator: { name: string };
    _count: { meals: number; assignments: number };
}

export default function OwnerDietsPage() {
    ...
    const columns: Column<DietPlan>[] = [
        {
            ...
            render: (p) => (
                <div>
                    ...
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest">{p._count.meals} Meals • {p.totalCalories} kcal</p>
                </div>
            ),
        },
        {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-slate-700">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Diet Plans</h1>
                    <p className="text-sm text-slate-500 font-medium mt-1">Design and assign nutritional templates for your members.</p>
                </div>
                <Link
                    href="/owner/diets/create"
            ...
            <div className="pt-4 flex flex-col gap-5">
                ...
                <DataTable<DietPlan>
                    ...
                    emptyIcon={<Utensils className="w-10 h-10 text-slate-300" />}
                    emptyTitle="No diet plans yet"
                    emptyDescription="Create your first systematic diet plan to assign to members."
                />
            </div>
...
function AssignModal({ planId, onClose, onSuccess }: { planId: string, onClose: () => void, onSuccess: () => void }) {
    ...
    const handleAssign = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            const res = await fetch("/api/diets/assign", {
                method: "POST",
            ...
            if (res.ok) {
                toast.success("Diet plan assigned effectively!");
                onSuccess();
            } else {
        ...
        <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="sm:max-w-md p-0 overflow-hidden border-slate-200 dark:border-slate-800">
                <DialogHeader className="px-8 py-6 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700">
                    <DialogTitle className="text-xl font-black text-slate-900 dark:text-white">Assign Diet Plan</DialogTitle>
                </DialogHeader>
