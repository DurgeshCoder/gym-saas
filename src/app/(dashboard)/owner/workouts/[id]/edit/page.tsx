    ...
    fetch(`/api/workouts/${id}`)
      ...
      .then((json) => {
        if (json.data) {
          ...
          if (plan.updatedAt) setLastUpdated(plan.updatedAt);
          const sanitizedData = {
            ...plan,
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {lastUpdated && (
                <Badge
                  variant="outline"
                  className="gap-1 border-border/60 text-muted-foreground font-normal text-[11px] py-0.5"
                >
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Modify the training protocol for your members.
            </p>
          </div>
        </div>
...
function DayCard({
  ...
  const watchedExercises =
    useWatch({ control, name: `days.${dayIndex}.exercises` }) || [];
          ...
          <div className="space-y-3">
            {exFields.map((exItem, exIndex) => (
              <ExerciseCard
                ...
                watchedVideo={watchedExercises[exIndex]}
              />
            ))}
          </div>
        </div>
...
// ─── Exercise Card (Edit variant — includes live video preview) ───────────────
    ...
    <div className="relative group rounded-xl border border-border/60 bg-card p-4 hover:border-border hover:shadow-sm transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3 border-t border-border/40">
        ...
        <div className="space-y-1.5">
          ...
          {/* Live preview */}
          {watchedVideo?.videoUrl && (
            <MediaPresenter
              videoUrl={watchedVideo.videoUrl}
              videoType={watchedVideo.videoType || "YOUTUBE"}
              compact
            />
          )}
        </div>
